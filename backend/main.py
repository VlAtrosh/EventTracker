...
@app.post("/api/events")
def create_event(event: EventCreate, admin=Depends(check_admin)):
    with get_connection() as conn:
        cursor = conn.execute("""
...
def register_event(event_id: int, current_user=Depends(get_current_user)):
    with get_connection() as conn:
...
def cancel_registration(event_id: int, current_user=Depends(get_current_user)):
    with get_connection() as conn:
        reg = conn.execute("""
            SELECT * FROM registrations 
            WHERE user_id = ? AND event_id = ? AND status = 'registered'
...
def my_registrations(current_user=Depends(get_current_user)):
    with get_connection() as conn:
        regs = conn.execute("""
...
def get_participants(event_id: int, admin=Depends(check_admin)):
    with get_connection() as conn:
